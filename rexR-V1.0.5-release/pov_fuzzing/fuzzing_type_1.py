...
_PREFILTER_BYTES = {"0", "1", "2", "A", "B", "k", "\xff", "\xca", "\x00"}
...
#	    print('not use angr: ',len(self.binaries))
    ...
    def analyze_bytes(self, byte_indices):
        ...
        if len(bytes_that_affect_regs) <= 1:
            return False
            ...
            while controlled_bits != 0:
                ...
                if number_bits > 8:
                    if self.analyze_complex(byte_indices, reg, bytes_to_regs):
                        ...
                        controlled_bits = self._filter_controlled_bits(controlled_bits, reg, bytes_to_regs, bit_indices)
                        continue
    ...
    def _filter_controlled_bits(self, controlled_bits, reg, bytes_to_regs, bit_indices):
        l.warning("fixing for fake complex")
